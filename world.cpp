...
void World::setup_zones(){
	...
	zones = new Zone[6];
	// intialize zones
	Zone singaproe, nyc, tokyo, mumbai, zurich, cairo;
	singapore = Zone(singapore_loc, "Singapore", 5400000, 716.1);
	nyc = Zone(nyc_loc, "New York City", 8405837, 1213.4);
	tokyo = Zone(tokyo_loc, "Tokyo", 13185502, 2187.66);
	mumbai = Zone(mumbai_loc, "Mumbai", 12478447, 603);
	zurich = Zone(zurich_loc, "Zurich", 383708, 87.88);
	cairo = Zone(cairo_loc, "Cairo", 10230520, 528);
	zones[0] = singapore;
	zones[1] = nyc;
	zones[2] = tokyo;
	zones[3] = mumbai;
...
void World::step() {
	if (step == 0) {
		...
		introduce_virus(hepatitis);
	}

	// TO DO, implement the various helper functions both here and the other classes
}
/* 
 * returns the zone with name we are looking for, or a nullptr if that zone does not exist
 */
Zone* World::get_zone(string name) {
	for (int i = 0; i < 5; i++) {
		if (zones[i].name.compares(name) == 0) {
			return &zones[i];
		}
	}
...
/* 
 * returns the virus with name we are looking for, or a nullptr if that zone does not exist
 */
Virus* World::get_virus(string name) {
